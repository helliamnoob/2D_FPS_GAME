                     ...
                     _In_ int       nCmdShow)
{
    ...
    wcex.hIcon = LoadIcon(hInstance, MAKEINTRESOURCE(IDI_MY2DFPS2));
...
#if 0
    int wW = wndWidth + win_border_width;
    // wW : wH = devSize.width : devSize.Height
    int wH = wndWidth * devSize.height / devSize.width + win_border_height;
#else
...
iPoint convertCoordinate(float x, float y)
{
    ...
    y -= rt.top;
#if 0
    printf("convertCoordinate(%.f, %.f) => (%.f, %.f)\n",
        x, y,
...
LRESULT CALLBACK wndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_CREATE:
    {
        ...
        if (hWnd == NULL)
            hWnd = hwnd;
#endif
        //win_border_width = GetSystemMetrics(SM_CXFRAME) * 2;
        //win_border_height = GetSystemMetrics(SM_CYCAPTION) + GetSystemMetrics(SM_CYFRAME);
        RECT rt0, rt1;
